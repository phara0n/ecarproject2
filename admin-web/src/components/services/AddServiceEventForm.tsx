...
const serviceEventSchema = z.object({
  ...
  date_scheduled: z.date({ required_error: "Date requise" }),
...
export function AddServiceEventForm({ 
    ...
    const fetchServiceTypes = async () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          const extractedTypes = Object.values(data).filter(value => 
  ...
  const onSubmit = async (data: ServiceEventFormValues) => {
    ...
    try {
      ...
      let tempSelectedVehicle = data.vehicle;
      ...
      try {
        ...
        console.error("[AddServiceEventForm] API error response:", error.response?.data);
